 ...
 * Direct, faithful implementation of mtkclient writeflash & dalegacy writeflash routines:
 * - mtkclient/Library/DA/xflash/xflash_lib.py: writeflash()
 * - mtkclient/Library/DA/dalegacy/dalegacy_lib.py: writeflash()
 */
class MtkFlashEngine(
    ...
    enum class UfsPartition(val value: Int) {
        LU0(MtkXFlashConstants.UfsPartitionType.LU0),
        LU1(MtkXFlashConstants.UfsPartitionType.LU1),
        LU2(MtkXFlashConstants.UfsPartitionType.LU2),
    ...
    companion object {
        ...
        const val FALLBACK_PACKET_SIZE = 0x100000 // 1MB fallback
    }
    ...
    /**
     * Calculates 16-bit additive checksum of binary payload:
     * sum(data) & 0xFFFF
     */
    ...
    /**
     * Flashes partition via XFlash Protocol (CMD_WRITE_DATA = 0x010004)
     * Real USB endpoint I/O - No simulation.
     */
    suspend fun flashXFlash(
        ...
        val packetSize = if (xflashEngine.maxPacketSize > 0) xflashEngine.maxPacketSize else FALLBACK_PACKET_SIZE

        log("[XFLASH] Flashing '${partition.partitionName}' @ 0x%X (Size: $totalLength bytes, Packet: ${packetSize / 1024} KB)".format(startAddr), LogLevel.INFO)
        log("[XFLASH] Target Storage: ${storageType.name} (0x%X), Section: 0x%X".format(storageType.value, partType), LogLevel.INFO)
    ...
    /**
     * Flashes partition via Legacy DA Protocol (0x62 SDMMC_WRITE_DATA)
     * Real USB endpoint I/O - No simulation.
     */
    suspend fun flashLegacy(
        ...
        storageType: StorageType = StorageType.EMMC,
        partType: Int = EmmcPartition.USER.value,
        chunkSize: Int = 0x20000 // 128KB chunks
    ): Boolean {
        val startAddr = partition.startLinearAddress
        val totalLength = data.size.toLong()
